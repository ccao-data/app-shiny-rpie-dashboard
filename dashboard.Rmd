...
# Create connections to CCAO and RPIE DB. Here connection credentials are loaded from env var
...
rpie_data <- qry_buildings(RPIE) %>%
  ...
  mutate(
    ...
    ~ ifelse(.x == 0 & (ResidentialFreeRent > 0 | ResOwnerOccupied > 0), NA, .x)
    ),
  # user submitted unit count and actual count of units can disagree, take larger count
  ResidentialUnitCount = pmax(ResidentialUnitCount, ResidentialUnits, na.rm = T),
  CommercialUnitCount = pmax(CommercialUnitCount, CommercialUnits, na.rm = T),
  ) %>%
...
palette <- c(
  ...
  ccao_colors$green, ccao_colors$brown, ccao_colors$gold, ccao_colors$lightgreen,
  "#e6550d"
) # red-orange
```
...
selectizeInput(
  ...
  choices = rpie_data %>% distinct(TaxYear) %>% rename("Reporting Year" = "TaxYear") %>% drop_na(),
  selected = NULL,
  multiple = TRUE
)
...
selectizeInput(
  ...
  label = "Classes",
  choices = rpie_data %>% distinct(class) %>% rename("Class" = "class") %>% drop_na() %>% arrange(Class),
  selected = NULL,
  multiple = TRUE
)
...
selectizeInput(
  ...
  label = "Triads",
  choices = rpie_data %>% distinct(triad_name) %>% rename("Triad" = "triad_name") %>% drop_na(),
  selected = NULL,
  multiple = TRUE
)
...
selectizeInput(
  ...
  label = "Townships",
  choices = rpie_data %>% distinct(township_name) %>% rename("Township" = "township_name") %>% drop_na() %>%
    arrange(Township),
  selected = NULL,
  multiple = TRUE
)
...
selectizeInput(
  ...
  choice = rpie_data %>% distinct(municipality) %>% rename("Municipality" = "municipality") %>% drop_na() %>%
...
selectizeInput(
  ...
  choices = rpie_data %>% distinct(ExpenseType) %>% rename("Tax Form" = "ExpenseType") %>% drop_na(),
  selected = NULL,
  multiple = TRUE
)
...
checkboxInput("free_rent_check", label = "Buildings with Free Rent", value = F)
checkboxInput("affordable_check", label = "Buildings with Affordable or Subsidized Units", value = F)
checkboxInput("own_occ_check", label = "Owner Occupied Buildings", value = F)
...
selected_data <- reactive({
  ...
  df <- rpie_data %>%
    filter(
      !is.na(!!sym(input$space_type_selector)), # convert the RHS into a symbol then evaluate (!!), filtering out NA values in selected column
  ...
  if (input$space_type_selector == "PIN") {
    df <- df %>% mutate(
      LegendChoice = case_when(
        ...
        # 500 class is broken out between industrial (5B) and commercial (5A)
        class %in% c(
          '500', '535', '501', '516', '517', '522', '523',
          '526', '527', '528', '529', '530', '531', '532',
          '533', '535', '590', '591', '592', '597', '599'
        ...
        TRUE ~ substr(class, 1, 1) # Everything else just takes the first digit as a character
      )
    )
  ...
  # but the ingested data is unique to the PIN level resulting in duplicate values based
  # on the # of associated PINs. This distinct call fixes that
  df <- df %>%
    group_by(BuildingId) %>%
    mutate(PIN = paste0(ccao::pin_format_pretty(PIN, full_length = TRUE), collapse = ", ")) %>%
...
renderLeaflet({
  ...
  if (input$space_type_selector == "PIN") legend_title <-
      paste0(format(nrow(selected_data()), big.mark = ","), " Buildings <hr> Major Class")
  if (input$space_type_selector == "CommercialUnitCount") legend_title <-
      paste0(format(nrow(selected_data()), big.mark = ","), " Buildings <hr> Commercial Rent / Square Foot <br> (by quartile)")
  if (input$space_type_selector == "ResidentialUnitCount") legend_title <-
      paste0(format(nrow(selected_data()), big.mark = ","), " Buildings <hr> Residential Rent / Square Foot <br> (by quartile)")
  ...
  leaflet(selected_data()) %>%
    ...
    addCircleMarkers(
      ...
      popup = ~ paste0(
        ...
        "<br/> <b> Residential Rent Per SF: </b>", scales::dollar(ResidentialRentPerSquareFoot),
        "<br/> <b> Commercial Units: </b>", CommercialUnitCount,
        "<br/> <b> Commercial Rent Per SF: </b>", scales::dollar(CommercialRentPerSquareFoot)
      ),
...
renderPlotly({
  ...
  df <- selected_data() %>%
    select(PIN,
      ...
      # remove outliers, 0 values, and other values that are too large to be realistic by # of bedrooms
      mutate(across(Studio:`4+ Bedroom`, purge_outliers)) %>% # purge_outliers() is defined in helper_functions.R
      mutate(across(Studio:`4+ Bedroom`, ~replace(., !dplyr::between(., 1, 5000), NA))) %>%
    pivot_longer(
    ...
    mutate(rental_space = factor(rental_space, levels = c("Studio", "1 Bedroom", "2 Bedroom", "3 Bedroom", "4+ Bedroom")))
  ...
  if (any(!is.na(df$rent))) {
      ...
      ggplotly(
        ggplot(
          df,
          aes(x = rental_space, y = rent, fill = rental_space, halign = 0,
              text = paste0("PIN(s): ", unique(substr(PIN, 1, 13)),
...
renderUI({
  ...
  summary_table <- selected_data() %>%
    mutate(ResidentialVacancy = ResidentialVacancies / ResidentialUnitCount,
         CommercialVacancy = CommercialVacancies / CommercialUnitCount) %>%
    mutate(ResidentialVacancy = ifelse(ResidentialVacancy <= 1, ResidentialVacancy, NA),
         CommercialVacancy = ifelse(CommercialVacancy <= 1, CommercialVacancy, NA)) %>%
    # we only want to show expense ratios using certain tax returns according to use input
    mutate(ExpenseRatio = ifelse(!is.null(input$expense_selector) & !(ExpenseType %in% input$expense_selector),
                                 NA,
                                 ExpenseRatio)) %>%
    ...
    # Remove outliers from ExpenseRatio, purge_outliers() is defined in helper_functions.R
    ...
    summarise(
      # and because of this it needs to be calculated separately from other summary stats
      across(-c(ResidentialVacancy, CommercialVacancy), ~ min(.x[.x >= 0.01], na.rm = T), .names = "{.col}_Minimum"),
      # vacancy minimum is likely to be 0 every time
      across(c(ResidentialVacancy, CommercialVacancy), ~ min(.x, na.rm = T), .names = "{.col}_Minimum"),

      # calculates all other summary statistics
      # # Not using snake case for initial variable names allows us to omit the above summaries using this match
      across(!contains("_"),
        # Names the functions in the output column headers
        list(Mean = mean, `Standard Deviation` = sd, Median = median, Max = max),
        .names = "{.col}_{.fn}",
        # Passes na.rm = T to all of the listed functions
        na.rm = T
      ),
    ...
    rationalize() %>%
    # Convert into a readable table with each variable or stat being a unique row
    pivot_longer(
      cols = everything(),
      # You can switch these two values around to transpose the table b/w variable/stat
    ...
    rename(
      ...
      "Commercial <br> (per SQFT, Yearly)" = "CommercialRentPerSquareFoot",
...
DT::renderDataTable(server = F, {
  # LegendChoice is a duplicated variable that doesn't need to be included in the download
  DT::datatable(
