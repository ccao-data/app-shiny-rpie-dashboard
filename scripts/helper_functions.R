# Function to clean address from PROPLOCS
...
# Function to find outliers for a given vector
...
# this function makes a lot of assumptions as to what the data it's given
# will look like and so will need to be adjusted as the report is adjusted
...
# Raw data needs to be cleaned before it's useful for the analysts to work with
# this function makes a lot of assumptions as to what the data it's given will
# look like and so will need to be adjusted as the report is adjusted
clean_expenses <- function(x, type = "general") {
  ...
  if (type == "general") {
    x <- x %>%
      ...
      mutate(across(
        TotalExpenseNet:ExpensesOther5Desc & !contains("Desc"),
        scales::dollar
      )) %>%
      rename(
        "Total Expenses Net of Int and Dep" = TotalExpenseNet,
    ...
    x <- x %>%
      ...
      mutate(across(
        RevenuePerRoom:UtilSanitServicesTotal & !contains("Name"),
        scales::dollar
      )) %>%
      rename(
        "Name of Hotel/Motel" = HotelName,
